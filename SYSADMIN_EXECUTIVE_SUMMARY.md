...
*   **Large Mailbox Readiness:** Startup paths are explicitly optimized for “power user” accounts (e.g., **50K+ contacts** and **15K+ folders**) using best-effort shim caches for expensive startup data (contact exports and `GetInfoRequest` folder/tag/prefs discovery). `GetInfoRequest` now serves a structurally valid cached response immediately even when stale, avoiding synchronous relogin rebuilds; only missing or unusable cache entries force a rebuild from upstream.
...
- Some ZWC features are intentionally partial or out of scope. Tasks and full classic zimlet parity are not yet implemented; Briefcase has initial file/folder support, basic checkout lock metadata, and read-only internal shared mounts, but not full Zimbra Docs, version-history, ZIP, rich-preview, shared-write, or external-share parity.
