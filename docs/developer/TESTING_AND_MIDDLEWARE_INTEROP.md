...
The product roadmap for turning this into a broader Zimbra-compatible application layer is
tracked in
[`PRODUCT_ZIMBRA_COMPATIBLE_APPLICATION_LAYER.md`](PRODUCT_ZIMBRA_COMPATIBLE_APPLICATION_LAYER.md).
That roadmap intentionally separates ZWC compatibility from third-party middleware
compatibility. A method working for ZWC is strong evidence, but it is not proof that every
Zimbra client or middleware product has the fields, faults, and response formats it expects.
...
Selected admin/provisioning APIs may be added later when a concrete partner or migration use
case exists and the behavior maps cleanly to Stalwart or bridge policy. The Zimlet runtime
itself remains out of scope even if some general-purpose APIs used by Zimlets are implemented.
