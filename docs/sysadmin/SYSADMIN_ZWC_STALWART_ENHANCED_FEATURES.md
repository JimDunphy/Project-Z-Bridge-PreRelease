...
- Rewrites remote images to authenticated same-origin proxy URLs. This rewrite
  does not prefetch images; remote hosts are contacted only if the browser later
  requests the proxy URL, such as after the user allows images.
...
2. Add operational training or runbook documentation under this directory if admins need to use or support it.
