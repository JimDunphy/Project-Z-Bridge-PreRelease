...
For public-Internet deployments behind nginx, also read: [`SYSADMIN_PRODUCTION_HARDENING.md`](SYSADMIN_PRODUCTION_HARDENING.md).
...
- **Download the supported release artifact** (recommended):
  ...
  - This downloads the latest supported `zimbra.war` from `example/DockerZimbraRHEL8` GitHub Releases into the repo root.
- **Build your own WAR manually** if you want a different/custom build:
  - Repo: `https://github.com/example/DockerZimbraRHEL8`
...
Note: Today the ZWC UI is mounted at `/zimbra/` for maximum compatibility. A future enhancement will allow configuring this mount point (example: `/mail/`) via an env var; see [`../developer/ARCHITECTURE_CONTEXT_PATH_PLAN.md`](../developer/ARCHITECTURE_CONTEXT_PATH_PLAN.md).
...
- Proxy **all** of these paths to the shim:
  ...
  - (Future) If/when the ZWC context path becomes configurable, proxy `/<context>/` instead of `/zimbra/` (see [`../developer/ARCHITECTURE_CONTEXT_PATH_PLAN.md`](../developer/ARCHITECTURE_CONTEXT_PATH_PLAN.md)).
...
- The source-repo runtime compose uses a named volume `bridge_data` for
  `BRIDGE_DATA_DIR=/data`.
- Runtime-only/evaluation checkouts may instead bind-mount host `./data` to
  `/data`. Treat that `./data` directory as persistent bridge-local state: it
  contains both rebuildable caches and durable ZWC-compatibility metadata such
  as signatures, identities, tag definitions/colors, and selected preferences.
  Back it up before host moves. For multi-node deployments, mount `./data` from
  a shared/distributed filesystem or equivalent persistent volume. See
  [`../developer/ARCHITECTURE_BRIDGE_USER_METADATA_STORE.md`](../developer/ARCHITECTURE_BRIDGE_USER_METADATA_STORE.md).
